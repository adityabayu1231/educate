...
@section('content')
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-semibold text-gray-900">Manage Kelas</h1>
            <button id="createKelasBtn"
                class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md flex items-center">
                <i class="fas fa-plus mr-2"></i> Create Kelas
            </button>
        </div>
            ...
            <table class="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
                <thead>
                    <tr class="bg-gradient-to-r from-gray-200 to-gray-300 text-gray-800 uppercase text-sm leading-normal">
                        ...
                        <th class="py-3 px-6 text-left">Kelas Name</th>
                    ...
                    @else
                        @foreach ($kelas as $item)
                            <tr class="border-b border-gray-200 hover:bg-gray-100">
                                ...
                                <td class="py-3 px-6 text-left">{{ $item->name_level }}</td>
                                <td class="py-3 px-6 text-right">
                                    <button data-id="{{ $item->id }}" data-name="{{ $item->name_level }}"
                                        class="editKelasBtn text-blue-500 hover:text-blue-700">
        ...
        <div id="createModal" class="fixed inset-0 items-center justify-center bg-gray-900 bg-opacity-50 hidden">
            <div class="bg-white rounded-lg p-6 w-full max-w-md">
                <h2 class="text-xl font-semibold mb-4">Create Kelas</h2>
                <form action="{{ route('admin.kelas.store') }}" method="POST">
                    @csrf
                    <div class="mb-4">
                        <label for="name_level" class="block text-gray-700">Kelas Name</label>
        ...
        <div id="editModal" class="fixed inset-0 items-center justify-center bg-gray-900 bg-opacity-50 hidden">
            <div class="bg-white rounded-lg p-6 w-full max-w-md">
                ...
                <form id="editForm" method="POST">
                    ...
                    @method('PUT')
                    <div class="mb-4">
                        <label for="edit_name_level" class="block text-gray-700">Kelas Name</label>
    ...
    @push('scripts')
        <script>
            ...
            document.getElementById('createKelasBtn').addEventListener('click', function() {
            ...
            document.querySelectorAll('.editKelasBtn').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    document.getElementById('edit_name_level').value = kelasName;
